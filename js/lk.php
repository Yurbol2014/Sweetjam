   ...
   <script>
      ...
      for (let i = 0; i < edit_buttons.length; i++) {
         ...
         edit_buttons[i].addEventListener("click", () => {
            edit_buttons[
               i
            ].previousElementSibling.innerHTML = `<input type="text" value="${inputValue}">`;
         ...
         save_buttons[i].addEventListener("click", async () => {
            let newInputValue = (edit_buttons[
                  i
               ].previousElementSibling.innerText =
               edit_buttons[i].previousElementSibling.firstElementChild.value);
            save_buttons[i].hidden = true;
            cancel_buttons[i].hidden = true;
            edit_buttons[i].hidden = false;
